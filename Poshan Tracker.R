...
# Clean up NFHS-5 output from Stata
...
# Merge NFHS-5 and Poshan Tracker data
growth.df <- left_join(growth.df,nfhs.df,by="state")
                                 
################################################################################
# AWCs functioning ####
################################################################################
tracker.df <- tracker.df %>% mutate(awc_15d_prop=round((awc_15d/awc_registered)*100,1),
                                  awc_21d_prop=round((awc_21d/awc_registered)*100,1))
...
# Receipt of THR ####
################################################################################
tracker.df <- tracker.df %>% mutate(thr_15d_prop=round((thr_15d/eligible_beneficiaries)*100,1),
                                    thr_21d_prop=round((thr_21d/eligible_beneficiaries)*100,1))

thr.long <- tracker.df %>% select(month,thr_15d_prop,thr_21d_prop) %>%
...
relabel.days_received = as_labeller(c(thr_15d_prop = "15 days", 
                                      thr_21d_prop = "21 days"))

thr.long <- thr.long %>% rowwise() %>% mutate(days_received2 = factor(relabel.days_received(days_received))) 

fig2.fig <- ggplot(thr.long, aes(x = month,y = proportion, group = days_received2, color = days_received2)) +
  geom_line() +  labs(title = "Proportion of eligible beneficiaries receiving 15 days or 21 days of THR over time",
                      x = "",
                      y = "Proportion of eligible beneficiaries (%)",
...
fig3.fig <- ggplot(stunting.df.long) +
...
ggsave(fig3.fig, filename = "Output/Stunting_NFHS5_PoshanTracker.jpeg",width = 9, height = 6)
...
fig4.fig <- ggplot(underweight.df.long) +
...
ggsave(fig4.fig, filename = "Output/Underweight_NFHS5_PoshanTracker.jpeg",width = 9, height = 6)
...
fig5.fig <- ggplot(wasting.df.long) +
...
ggsave(fig5.fig, filename = "Output/Wasting_NFHS5_PoshanTracker.jpeg",width = 9, height = 6)
...
fig6.fig <- ggplot(overweight.df.long) +
...
ggsave(fig6.fig, filename = "Output/Overweight_NFHS5_PoshanTracker.jpeg",width = 9, height = 6)
